# WorkoutTracker

A full-stack workout tracking web application to log, view, and analyze your fitness routines.

[![Live Demo](https://img.shields.io/badge/Live%20Demo-Visit%20Now-brightgreen)](https://workouttracker-112.runasp.net)

**Backend**: .NET (C#) API  
**Frontend**: HTML + CSS + JavaScript  
**Architecture**: RESTful API + static frontend

## ✨ Features

- Create, read, update, and delete workout sessions
- Log exercises with sets, reps, weight, and notes
- View workout history and basic statistics
- Simple, clean, responsive user interface
- (planned) User authentication & multiple profiles
- (planned) Progress charts and export functionality

## 📸 Screenshots

### Mobile Views

<div align="center">

<img src="ScreenShots/Mobile_dash.jpeg" 
     alt="Dashboard - Weekly overview, start button, recent activity" 
     width="340" 
     style="margin: 30px 0;">

<img src="ScreenShots/Mobile_WorkoutLog.jpeg" 
     alt="Active workout - Bench Press with sets, reps, timer" 
     width="340" 
     style="margin: 30px 0;">

<img src="ScreenShots/Mobile_Template.jpeg" 
     alt="Workout Planner - Templates list like Pull and Upper" 
     width="340" 
     style="margin: 30px 0;">

<img src="ScreenShots/Mobile_ProgressBar.jpeg" 
     alt="Progress - Weekly bar chart, totals, streak" 
     width="340" 
     style="margin: 30px 0;">

</div>

<p align="center"><em>FitTrack mobile interface (dark mode)</em></p>

## 🛠️ Tech Stack

- **Backend**  
  - .NET 8   
  - ASP.NET Core Web API  
  - Entity Framework Core

- **Frontend**  
  - HTML5  
  - CSS3
  - JavaScript

- **Tools**  
  - Visual Studio 
  - Git
  - MSSQL



